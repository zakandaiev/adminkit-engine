class ForeignForm {
	...
	initialize() {
		...
		this.button.open_modal.addEventListener('click', event => {
			if(!this.is_editing) {
	...
	clickSubmit() {
		...
		if(this.is_editing) {
			this.inputs.forEach(input => {
				this.editing_row.querySelector(`[data-name="${input.name}"]`).textContent = input.value;
			});
	...
	clickEdit(trow) {
		this.inputs.forEach(input => {
			const value = trow.querySelector(`[data-name="${input.name}"]`).textContent;
