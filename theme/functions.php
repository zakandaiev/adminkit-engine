...
function get_menu($key) {
	return Model::getInstance()->getMenu($key);
}
...
function menu_aside($menu) {
	...
	foreach($menu as $item) {
		...
		if(!empty($item['children'])) {
			...
			$output .= menu_aside($item['children']);
			$output .= '</ul>';
		}
...
function menu_social_header($menu) {
	foreach($menu as $item) {
		echo '<li><a href="' . $item['url'] . '" target="_blank"><i class="fa fa-' . strtolower($item['name']) . '"></i></a></li>';
	}
}

function menu_social_footer($menu) {
	foreach($menu as $item) {
		echo '<li><a href="' . $item['url'] . '" class="social-' . strtolower($item['name']) . '" target="_blank"><i class="fa fa-' . strtolower($item['name']) . '"></i></a></li> ';
	}
}
